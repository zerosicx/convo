...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center h-8 gap-2 p-1 text-sm font-medium transition-all hover:bg-zinc-700 text-left [&[data-state=open]>svg]:rotate-90 ring-0 outline-none focus:outline-none focus-visible:outline-none border-0 bg-transparent [&[data-state=open]]:bg-brand rounded-sm",
        className
      )}
      {...props}
    >
      <ChevronRight className="h-4 w-4 shrink-0 text-zinc-100 transition-transform duration-200" />
      {children}
    </AccordionPrimitive.Trigger>
