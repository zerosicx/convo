        ...
        <>
            ...
            }
            {
                creatingSection ? <div className="w-full">
                    <Input autoFocus ref={sectionInputRef} placeholder={"Untitled Section"} style={{ maxWidth: '100%' }} onKeyDown={(event) => {
    ...
    return (
        <NavLink to={`notebook/${notebookId}/section/${section.id}`}>
            <div className={cn("text-left p-1 text-primary flex flex-row justify-between items-center",
                ...
                {
                    editing ? <Input autoFocus variant="transparent" placeholder={section.name} onKeyDown={handleKeyDown} onChange={handleChange} onBlur={() => setEditing(false)} /> : <span>{section.name}</span>
                }
                <DropdownMenu>
